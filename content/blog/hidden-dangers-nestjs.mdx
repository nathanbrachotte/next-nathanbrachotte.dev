...
{/* Original: Pasted image 20231208231535.png */}
...
Pass anything as a query parameter with code that doesn't specify a DTO, and your `query` will just be whatever you passed—no questions asked.
...
## Danger #4: Class Validator—the problematic default
...
NestJS's reliance on classes—a pattern now considered outdated by many in the JavaScript community—poses scalability issues.
...
It seems like what NestJS should have been—a serious, well-documented framework that doesn't cut corners.
...
In conclusion, while NestJS offers a structured entry point into the TypeScript/JavaScript world, it comes with hidden complexities that can escalate into significant challenges—especially for teams without seasoned NestJS developers.
...
<ProsCard
  ...
  pros={[
    ...
    "Don't trust the docs—always test edge cases",
