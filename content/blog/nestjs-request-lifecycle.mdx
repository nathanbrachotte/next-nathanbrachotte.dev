...
summary: 'Too many Engineers still do not fully understand what piece of code is going to run first when hitting your NestJS backend. Lets go through into all the components that make a request lifecycle'
...
We also added a way to log the context in which the Guard is being executed, this is useful because we now can add the Guard at different levels in the app:
...
**That means that both `app.useGlobalGuard()` and `@UseGuards()` ultimately register the Guard at the same level! We should consider them at the same level and not rely on one being triggered before the other.**
...
They can, for example, be used to monitor how long your request took to return to your client. They could also be used to implement a caching mechanism on some of your routes, for example.
...
<ProsCard
  ...
  pros={[
    ...
    'In the case of Filters, the one closest to the error is the one that is triggered.You want the lowest barrier to entry.',
  ]}
/>
