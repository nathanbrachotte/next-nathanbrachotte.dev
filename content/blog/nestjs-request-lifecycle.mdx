...
Middlewares are pretty effective, but they have a major drawback: they are not aware of the context of the request. They are executed before the controller, and therefore have no way to know what controller or method is being called. This is where Guards come in.
...
Now, let's send a first request without the header and see which is triggered first:
