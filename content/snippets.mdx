...
export function getStatusLabel(id: StatusId): string {
  return STATUS[id.toUpperCase() as keyof typeof STATUS].label;
}
...
- Use `getValues`/`getEntries`/`getKeys` from utils for type-safe Object methods (@see https://www.nathanbrachotte.dev/snippets#type-safe-object-entries)
